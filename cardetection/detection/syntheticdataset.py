...
def draw_circle(img, bounding_rect, filled=False):
    ...
    if filled:
        lw = -1
    radius = int(ry*0.95 - lw*0.5)

    col = random_colour()
    cv2.circle(img, centre, radius, col, lw)

def draw_rectangle(img, bounding_rect):
...
def draw_background_object(img, bounding_rect):
    if np.random.uniform() > 0.25:
        draw_rectangle(img, bounding_rect)
    else:
        draw_circle(img, bounding_rect, filled=True)

def draw_positive_object(img, bounding_rect):
    draw_circle(img, bounding_rect, filled=False)
...
def synthesise_background_image(img_dims=(800, 600), num_objects=1000):
    ...
    for _ in xrange(num_objects):
        # rect = gm.PixelRectangle.random_with_same_aspect(approx_window_dims, img_dims)
...
def synthesise_positive_image(min_window_dims, objects_per_image=5, img_dims=(800, 600)):
    ...
    for _ in xrange(objects_per_image):
        ...
        rect = gm.PixelRectangle.random_with_same_aspect(min_window_dims, img_dims)
        draw_positive_object(img, rect)
        rects.append(rect)

    return img, rects

def synthesise_dataset(base_dir, pos_num, neg_num, test_num, objects_per_image):
    ...
    #
    # Synthesise test images:
    # test_bbinfo_map = {}
    for index in xrange(test_num):
        img, rects = synthesise_positive_image((150, 100), objects_per_image)

        fname = os.path.join(test_dir, '{:06d}.png'.format(index))
        # test_bbinfo_map[fname] = rects
        cv2.imwrite(fname, img)
    #
